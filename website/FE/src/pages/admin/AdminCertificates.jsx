...
const CERTIFICATE_ENDPOINT =
  'https://arduflow.indobilliard.com/apk/uploads/web/api/certificate-api.php';

const CERTIFICATE_UPLOAD_ENDPOINT =
  'https://arduflow.indobilliard.com/apk/uploads/web/api/certificate-upload.php';

console.log('[AdminCertificates] CERTIFICATE_ENDPOINT:', CERTIFICATE_ENDPOINT);
console.log('[AdminCertificates] CERTIFICATE_UPLOAD_ENDPOINT:', CERTIFICATE_UPLOAD_ENDPOINT);
...
function parseMemberNames(value) {
  return String(value || '')
    ...
    .map((line) => {
      ...
      // 1. Nama Peserta <email@domain.com>
      // Nama Peserta <email@domain.com>
      // Nama Peserta
...
function expandParticipantMembers(participant) {
  const parsedMembers = parseMemberNames(participant.memberNames);
  ...
  return members.map((member, index) => {
    const memberName = member.name || participant.participantName;
    const memberEmail =
      member.email ||
    ...
    return {
      ...
      // Email sertifikat diarahkan ke email member jika tersedia.
      participantEmail: memberEmail,

      registrationParticipantName: participant.participantName,
      hasExplicitMemberList: parsedMembers.length > 0,
      memberIndex: index,
    };
  });
}
...
function CertificateFormModal({
  ...
  const handleWorkshopChange = (event) => {
    ...
    onChange({
      ...
      // Jenis sertifikat, bukan kategori workshop.
      type: 'Workshop',
...
export function AdminCertificates() {
  ...
  const createCertificateForParticipant = async (
    ...
    const payload = {
      ...
      // Backend hanya menerima: Workshop, Program, atau Course.
      // Karena data pada halaman ini berasal dari workshop, gunakan Workshop.
      type: 'Workshop',
    ...
    console.log('[Certificate Generate] payload:', payload);
  ...
  const handleUploadCertificate = async (certificate, file, options = {}) => {
    ...
    try {
      const uploadUrl = new URL(
        CERTIFICATE_UPLOAD_ENDPOINT,
        window.location.origin,
      );
      uploadUrl.searchParams.set('id', certificate.id);

      console.log('[Certificate Upload] endpoint:', uploadUrl.toString());
  ...
  const openGenerateWorkshopCertificates = (row) => {
    ...
    setForm({
      ...
      // row.category adalah kategori seperti Arduino/IoT, bukan jenis sertifikat.
      type: 'Workshop',
