...
{
    private const VERSION = '010_legacy_api_schema';
    ...
    private function contentLegacy(PDO $pdo): void
    {
        ...
        $this->ensureColumns($pdo, 'tutorials', [
        ...
        $this->ensureTable(
            ...
            'CREATE TABLE tutorial_slides (
                ...
                updated_at TEXT NOT NULL,
                FOREIGN KEY (tutorial_id) REFERENCES tutorials(id) ON DELETE CASCADE,
                FOREIGN KEY (chapter_id) REFERENCES tutorial_chapters(id) ON DELETE SET NULL
            )'
        );

        $this->ensureColumns($pdo, 'tutorial_slides', [
            'chapter_id' => 'INTEGER',
            'estimated_time' => 'TEXT',
            'status' => 'TEXT NOT NULL DEFAULT "draft"',
            'image_type' => 'TEXT',
        ...
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_tutorial_chapters_tutorial ON tutorial_chapters(tutorial_id, chapter_order)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_tutorial_objectives_tutorial ON tutorial_learning_objectives(tutorial_id, objective_order)');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_tutorial_slides_chapter ON tutorial_slides(tutorial_id, chapter_id, slide_order)');
    }
