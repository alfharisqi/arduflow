...
/** Additive maintenance only. Never called from an HTTP request. */
function afwApiQueryIndexes(): array
{
    return [
        // Match LOWER()/NOCASE expressions actually used by legacy queries.
...
function afwInstallApiQueryIndexes(PDO $pdo): array
{
    $existing = array_fill_keys($pdo->query("SELECT name FROM sqlite_master WHERE type = 'index'")->fetchAll(PDO::FETCH_COLUMN), true);
    $columnsByTable = [];
    $result = ['created' => [], 'existing' => [], 'skipped' => []];
    foreach (afwApiQueryIndexes() as $name => [$table, $required, $expression, $where]) {
        if (isset($existing[$name])) {
            $result['existing'][] = $name;
            continue;
        }
        if (!isset($columnsByTable[$table])) {
            $columnsByTable[$table] = array_column($pdo->query('PRAGMA table_info("' . $table . '")')->fetchAll(PDO::FETCH_ASSOC), 'name');
        }
        if (array_diff($required, $columnsByTable[$table]) !== []) {
            $result['skipped'][] = $name;
            continue;
        }
        $pdo->exec('CREATE INDEX IF NOT EXISTS "' . $name . '" ON "' . $table . '" (' . $expression . ')'
            . ($where === '' ? '' : ' WHERE ' . $where));
        $result['created'][] = $name;
    }
    return $result;
