...
$stubs = 'function sendJsonResponse(array $payload, int $status = 200): void { throw new \ApiCapturedResponse($payload); }
    function getCardImagePath(?string $name): ?string { return $name === null ? null : "/cards/" . $name; }
    function getCardImageUrl(?string $name): ?string { return getCardImagePath($name); }
...
$pdo->exec('CREATE TABLE materi_chapters (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
$pdo->exec('CREATE TABLE materi_learning_objectives (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    materi_id INTEGER NOT NULL,
...
$pdo->exec('CREATE TABLE materi_slides (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    materi_id INTEGER NOT NULL,
    chapter_id INTEGER,
...
for ($id = 1; $id <= 100; $id++) {
    $pdo->exec("INSERT INTO materi (id, title, slug, category, short_description, full_description, page_order, display_order, created_at, updated_at)
        VALUES ($id, 'Materi $id', 'materi-$id', 'IoT', 'Ringkas', 'Lengkap', 1, " . ($id % 3) . ", '2026-01-01', '2026-01-02')");
    if ($id !== 1) {
        $pdo->exec("INSERT INTO materi_chapters (id, materi_id, chapter_order, title, created_at, updated_at)
            VALUES ($id, $id, 1, 'Bab', '2026-01-01', '2026-01-02')");
    }
    $pdo->exec("INSERT INTO materi_slides (materi_id, slide_order, title, content_type, content, created_at, updated_at)
    ...
    $pdo->exec("INSERT INTO materi_learning_objectives (materi_id, objective_order, objective, created_at, updated_at)
