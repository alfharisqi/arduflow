...
- Jalankan server dari root `website/BE` agar semua path config, storage, dan autoload konsisten.
...
```cron
*/5 * * * * cd /path/to/arduflow/website/BE && php scripts/sync-sqlite-to-mysql.php >> storage/logs/sync-cron.log 2>&1
```
